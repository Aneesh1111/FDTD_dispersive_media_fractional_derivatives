...
nmax = 1001*off; % max discretization in time
isource = 200*off;
i_media_start = 300*off;
i_media_stop = 350*off;
...
for n = 2:nmax
    ...
    if mod(n,10) == 0
        ...
        plot(real(Ex(n,:)));
        hold on
        xline(i_media_start, 'Color', [0.8500 0.3250 0.0980]);
        xline(i_media_stop, 'Color', [0.8500 0.3250 0.0980]);
        xline(PML_thickness);
        xline(imax-PML_thickness);
%         rectangle('Position', [i_media_start,-1,i_media_start,2])
        hold off
        txt = {'Dielectric'};
        text(290,0.6,txt)
        ...
        ylabel('$\Re e\{E_x\}$', 'Interpreter', 'latex')
...
filename = '1D_FDTD_animation.gif';
